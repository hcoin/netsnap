...
A comprehensive package for pulling nearly all details of Linux networking into reliable and broadly usable form. From configuration to statistics, using the fastest available api: RTNetlink and Generic Netlink protocols. Can fuction in standalone fashion generating JSON output, or as objects within Python 3.8+. NetMon provides deep visibility into network interfaces, routing tables, neighbor tables, multicast databases, and routing rules through direct kernel communication via CFFI. Different than most alternatives: instead of kernel constants hard coded and duplicated in alternatives to NetMon from the then current kernel version on developer's systems, NetMon enhances portability and maintainability across distros and kernel releases by referencing the kernel running on each system as the 'single source of truth' for all symbolic definitions.

In use cases where networking configuration changes happen every second and each change must be tracked in real time, consider alternatives such as pyroute2. NetMon provides very low-overhead, yet very very complete snapshots of each of the major Linux networking subsystems (setup of devices of all sorts, routes, rules, and link-layer tables).
